...
def pbBattleOrSpawnOnStepTaken(repel_active)
  # Si el mapa no está permitido para OW, fuerza encuentros normales
  unless VisibleEncounterSettings::ALLOWED_MAP_IDS.include?($game_map.map_id)
    return true # Forzar encuentros normales
  end
...
def pbSpawnOnStepTaken(repel_active)
  # Verifica si el mapa actual está en la lista permitida
  unless VisibleEncounterSettings::ALLOWED_MAP_IDS.include?($game_map.map_id)
    return # Sal del método si el mapa no está permitido
  end

  return if $player.able_pokemon_count == 0 # Verifica si el jugador tiene Pokémon
