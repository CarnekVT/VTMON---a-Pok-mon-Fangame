class Battle::Battler
    ...
    def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
      ...
      if self.status == newStatus && !ignoreStatus
        if showMessages
          ...
          when :GLITCH    then msg = _INTL("¡{1} ya está corrompido!", pbThis)
          end
      ...
      if immuneByAbility
        if showMessages
          ...
          if Battle::Scene::USE_ABILITY_SPLASH
            ...
            when :GLITCH    then msg = _INTL("¡{1} no puede ser corrompido!", pbThis)
            end
          elsif immAlly
            case newStatus
            when :SLEEP
            ...
            when :FROZEN
              msg = _INTL("¡{1} no puede ser congelado because of {2}'s {3}!",
                          pbThis, immAlly.pbThis(true), immAlly.abilityName)
            when :GLTICH
               msg = _INTL("¡{1} no puede ser corrompido por la habilidad {3} de {2}!",
                          pbThis, immAlly.pbThis(true), immAlly.abilityName)              
            end
          else
            case newStatus
            ...
            when :GLITCH    then msg = _INTL("¡La habilidad {1} de {2} previene la corrupción!", pbThis, abilityName)
            end
          end
          @battle.pbDisplay(msg)
    ...
    def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
      ...
      if msg && !msg.empty?
        ...
        when :GLITCH
            @battle.pbDisplay(_INTL("¡{1} está corrupto!", pbThis))
        end
      end
    ...
    #=============================================================================
    def glitch?
        return pbHasStatus?(:GLITCH)
      end
    
      def pbCanGlitch?(user, showMessages, move = nil)
        return pbCanInflictStatus?(:GLITCH, user, showMessages, move)
      end
    
      def pbCanGlitchSynchronize?(target)
        return pbCanSynchronizeStatus?(:GLITCH, target)
      end
    
      def pbGlitch(user = nil, msg = nil)
        pbInflictStatus(:GLITCH, 0, msg, user)
      end
    ...
    def pbContinueStatus
      ...
      when :GLITCH
        @battle.pbDisplay(_INTL("¡El {1} se resiente de la corrupción!", pbThis))
      end
    ...
    def pbCureStatus(showMessages = true)
      ...
      if showMessages
        ...
        when :GLITCH    then @battle.pbDisplay(_INTL("{1} se ha curado de la corrupción.", pbThis))
        end
      end
