  class Battle
    ...
    # Damage from glitch
    priority.each do |battler|
    next if battler.status != :GLITCH || !battler.takesIndirectDamage?
    battler.droppedBelowHalfHP = false
    dmg = battler.totalhp / 16
    battler.pbContinueStatus { battler.pbReduceHP(dmg, false) }
    battler.pbItemHPHealCheck
    ...
    def pbEndOfRoundPhase
      ...
      if Settings::AFFECTION_EFFECTS && @internalBattle
        priority.each do |battler|
          ...
          when :GLITCH
            pbDisplay(_INTL("¡{1} curó el virus con su pura determinación para no decepcionarte!", battler.pbThis))
          end
        end
      end
